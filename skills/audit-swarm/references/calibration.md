...
2. Spawn ONE seeding agent (general-purpose, model sonnet) with this task, verbatim:
   "In the worktree at <path>, plant exactly <N=5-6> realistic bugs, one per class
   ...
   should cover; make a docstring lie about behavior>. Each bug must be subtle
   (survives a skim), syntactically valid, and NOT break the build. Write the answer
   key — file:line, class, one-line description per seed — as your final message.
